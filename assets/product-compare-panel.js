...
if (!customElements.get('product-compare-panel')) {
	class ProductComparePanel extends HTMLElement {
		constructor() {
			...
			this.modal = document.querySelector('.product-compare-modal');
			this.table = this.modal.querySelector('product-compare-table');
			this.max = Number(this.dataset.max);
			this.href = '';
		}

		connectedCallback() {
			localStorage.removeItem('compare-products-vision');
