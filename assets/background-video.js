...
if (!customElements.get('background-video')) {
  class BackgroundVideo extends HTMLElement {
    constructor() {
      ...
      this.poster = this.querySelector('.background-video .background-video__fallback-img');
    }
    connectedCallback() {
      let _this = this;
      ...
      if (this.video_container.querySelector('iframe')) {
        this.video_container.querySelector('iframe').onload = function() {
          // _this.videoPlay();  // removed autoplay
        };
      }
    ...
    setupControlEvents() {
      if (this.paused) {
        ...
        if (this.toggle) {
          ...
          this.poster.classList.remove('fallback-img-wrap');
        }
      } else {
        this.videoPause();
        if (this.toggle) {
          this.toggle.classList.add('paused');
          // this.poster.classList.add('fallback-img-wrap');
        }
      }
    }
    videoPlay() {
...
document.querySelectorAll('.background-video__controls').forEach(controls => {
  const btn = controls.querySelector('button');
  const contentWrap = controls.closest('.background-video').querySelector('.background-video__content-wrap');
  const contentover = controls.closest('.background-video').querySelector('.background-video__overlay');

  function updateActiveClass() {
    if (btn.classList.contains('paused')) {
      contentWrap.classList.add('active');
      contentover.classList.add('active');
    } else {
      contentWrap.classList.remove('active');
      contentover.classList.remove('active');
    }
  }

  updateActiveClass();
  btn.addEventListener('click', updateActiveClass);
});
