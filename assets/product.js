...
if (!customElements.get('product-form')) {
  customElements.define('product-form', class ProductForm extends HTMLElement {
    ...
    onSubmitHandler(evt) {
      ...
      const warrantyCheckboxes = document.querySelectorAll('.warranty-checkbox:checked');

      // ✅ Always convert to items API
      let itemsIndex = 0;

      // ✅ MAIN PRODUCT ADD
      const mainVariantId = this.form.querySelector('[name="id"]').value;

      formData.delete('id'); // default remove
      
      let qty = 1;

      const qtyInput = this.closest('.quantity-add')?.querySelector('.quantity.buttons_added input[type="number"]');
      ...
      formData.append(`items[${itemsIndex}][id]`, mainVariantId);
      formData.append(`items[${itemsIndex}][quantity]`, qty);

      itemsIndex++;

      // ✅ WARRANTY PRODUCTS ADD (ONLY CHECKED)
      warrantyCheckboxes.forEach((checkbox) => {
        const warrantyVariantId = checkbox.dataset.variantId;
        if (!warrantyVariantId) return;

        formData.append(`items[${itemsIndex}][id]`, warrantyVariantId);
        formData.append(`items[${itemsIndex}][quantity]`, 1);
...
document.addEventListener('DOMContentLoaded', function () {
  const checkboxes = document.querySelectorAll('.warranty-checkbox');
  if (checkboxes) {
